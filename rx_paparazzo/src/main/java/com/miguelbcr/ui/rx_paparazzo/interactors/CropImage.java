...
public final class CropImage extends UseCase<Uri> {
  ...
  @Override public Observable<Uri> react() {
    if (config.doCrop()) {
      return getIntent().flatMap(new Func1<Intent, Observable<Uri>>() {
        @Override public Observable<Uri> call(Intent intent) {
          intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
          return startIntent.with(intent).react().map(new Func1<Intent, Uri>() {
            @Override public Uri call(Intent intentResult) {
  ...
  private Observable<Intent> getIntent() {
    return Observable.zip(getInputUri(), getOutputUriCrop(), new Func2<Uri, Uri, Intent>() {
      @Override public Intent call(Uri uri, Uri outputUri) {
  ...
  private Observable<Uri> getInputUri() {
    return getPath.with(uri).react().map(new Func1<String, Uri>() {
      @Override public Uri call(String filePath) {
  ...
  private Observable<Uri> getOutputUriCrop() {
    return getPath.with(uri).react().flatMap(new Func1<String, Observable<Uri>>() {
      @Override public Observable<Uri> call(String filepath) {
